...
public:
  ...
  buffer(const vku::device &device, void *init, VkDeviceSize size, VkBufferUsageFlags usage) : resource(device) {
    ...
    err = vkAllocateMemory(dev(), &memAlloc, VK_NULL_HANDLE, &aux_.mem);
    if (err) throw error(err, __FILE__, __LINE__);
    ...
    err = vkBindBufferMemory(dev(), get(), aux_.mem, 0);
    if (err) throw error(err, __FILE__, __LINE__);
  }
  ...
  void unmap() const {
    vkUnmapMemory(dev(), aux_.mem);
  }

  size_t size() const {
    return (size_t)aux_.size;
  }
